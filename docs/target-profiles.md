...
```json
{
  ...
  "interface_renaming_rules": {
    ...
    "target_prefix": "TeGigabitEthernet",
    "range_start": 1,
    "range_end": 48
  },
...
* `target_prefix`: The media speed string for your new hardware (e.g., `TeGigabitEthernet` for 10G, `TwentyFiveGenE` for 25G).
...
The mapping engine is built with safety overrides to ensure bad configurations are caught before they touch your live network. If your JSON profile introduces logical errors, the GUI's **Audit Panel** will throw explicit warnings:

* **Critical Collisions**: If your layout accidently points two different legacy source ports to the exact same new destination port, the tool flags both lines with a clear error block.
